...
export default function CspsgDashboard() {
  ...
  const loadStats = useCallback(async () => {
    ...
    try {
      ...
      const [cspsgOrg, settingsData] = await Promise.all([
      ...
      // Build map of latest request per student
      ...
      // Fetch CSPSG-specific clearance items for these requests
      const requestIds = currentRequests.map((r) => r.id);
      const orgItems = requestIds.length > 0
        ? await getClearanceItemsBySourceAndRequests("cspsg", cspsgOrg.id, requestIds)
        : [];
      ...
      const enrichedStudents: StudentWithStatus[] = students.map((student) => {
        ...
        const orgItem = req ? itemByRequest.get(req.id) ?? null : null;
        ...
        return { ...student, latestRequest: req, orgItem, status };
      });
      ...
      console.error("Error loading CSPSG stats:", err);
    ...
    <div className="min-h-screen bg-surface-warm">
      ...
      <div className="p-6 space-y-6">
        ...
        <div className="flex flex-wrap gap-2 justify-center text-sm text-warm-muted">
          ...
          <span className="px-3 py-1 bg-white border border-gray-200 rounded-full">
            {profile?.role === "cspsg" ? "CSPSG Head" : "Staff"}
          </span>
          <span className="px-3 py-1 bg-white border border-gray-200 rounded-full">
