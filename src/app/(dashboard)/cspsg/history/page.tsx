...
function ItemStatusBadge({
  ...
  status: ClearanceItemWithDetails["status"];
}) {
  switch (status) {
    case "approved":
      return (
...
function SubmissionStatusBadge({
  ...
  status: SubmissionWithRequirement["status"];
}) {
  switch (status) {
    case "verified":
...
export default function CspsgHistoryPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      if (!cspsgOrg) {
        setError("Your CSPSG organisation could not be found.");
        setItems([]);
        return;
      }
      setOrg(cspsgOrg);
      ...
      const { data, error: dbError } = await supabase
        ...
        .select(`*, request:clearance_requests(*, student:profiles(*))`)
        .eq("source_type", "cspsg")
        .eq("source_id", cspsgOrg.id)
        .in("status", ["approved", "rejected", "on_hold"])
        .order("reviewed_at", { ascending: false });

      if (dbError) throw dbError;
      setItems((data as ClearanceItemWithDetails[]) ?? []);
    } catch (err) {
            ...
            onClick={() => {
              exportToExcel(
                ...
                [
                  {
                    header: "Student ID",
                    accessor: (i) => i.request?.student?.student_id ?? "—",
                  },
                  {
                    header: "Name",
                    accessor: (i) => {
                      const s = i.request?.student;
                      return s
                  ...
                  },
                  {
                    header: "Type",
                    accessor: (i) => {
                      const t = i.request?.type ?? "semester";
                  ...
                  },
                  {
                    ...
                    accessor: (i) =>
                      i.request
                        ? `${i.request.academic_year} — ${i.request.semester}`
                        : "—",
                  },
                  { header: "Status", accessor: (i) => i.status },
                  {
                    ...
                    accessor: (i) =>
                      i.reviewed_at ? formatDate(i.reviewed_at) : "—",
                  },
                ],
                `${org?.name ?? "cspsg"}_clearance_history`
              );
            }}
      ...
      >
        {selectedItem && (
          <div className="p-6 space-y-6">
            ...
            <div>
              <h3 className="text-sm font-semibold text-cjc-navy mb-3">
                Status History
              </h3>
