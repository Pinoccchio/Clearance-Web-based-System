...
export default function CspsgProfilePage() {
  ...
  const loadOrg = useCallback(async () => {
    ...
    setOrg(cspsgOrg);
    setOrgForm({ description: cspsgOrg?.description ?? "" });
  ...
  useRealtimeRefresh("cspsg", loadOrg);
  useRealtimeRefresh("profiles", loadOrg);
  ...
  const handleSaveOrg = async () => {
    ...
    setIsSavingOrg(true);
    try {
      const updated = await updateCspsg(org.id, {
      ...
      showToast(
        "success",
        "CSPSG Updated",
        "CSPSG information has been saved."
      );
    } catch {
      showToast(
        "error",
        "Update Failed",
        "Could not save CSPSG info. Please try again."
      );
    } finally {
  ...
  const handleCancelOrg = () => {
    setOrgForm({ description: org?.description ?? "" });
    setIsEditingOrg(false);
  };
  ...
  const handleLogoUpload = async (file: File) => {
    ...
    try {
      if (org.logo_url) await deleteLogo(org.logo_url);
      const url = await uploadLogo(file, "cspsg", org.id);
      const updated = await updateCspsg(org.id, { logo_url: url });
      setOrg(updated);
      showToast("success", "Logo Updated", "CSPSG logo has been changed.");
    } catch {
      showToast(
        "error",
        "Upload Failed",
          ...
          <div className="lg:col-span-1">
            <Card padding="lg" className="text-center">
              ...
              <div className="relative inline-block mb-4">
                {org?.logo_url ? (
                  ...
                  <img
                    ...
                    onClick={() => org.logo_url && setPreviewUrl(org.logo_url)}
                  />
                ) : (
              ...
              <div className="mt-6 pt-6 border-t border-gray-100 text-left">
                ...
                <Badge variant="info">CSPSG Head</Badge>
              </div>
            </Card>
          </div>
              ...
              <div className="flex items-center gap-4 mb-6">
                <div className="relative inline-block">
                  <Avatar
                    ...
                    className={cn(
                      "w-16 h-16 text-xl",
                      profile?.avatar_url && "cursor-pointer"
                    )}
                    onClick={() =>
                      profile?.avatar_url && setPreviewUrl(profile.avatar_url)
                    }
                  />
                  <label
                ...
                <div className="grid sm:grid-cols-2 gap-4">
                  ...
                  <div className="sm:col-span-2">
                    ...
                    <textarea
                      ...
                      onChange={(e) =>
                        setOrgForm({ description: e.target.value })
                      }
                      disabled={!isEditingOrg}
                      placeholder="Enter CSPSG description..."
                    />
                  </div>
                </div>
              ) : (
