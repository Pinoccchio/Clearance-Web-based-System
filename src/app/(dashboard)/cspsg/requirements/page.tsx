...
export default function CspsgRequirementsPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      if (!cspsgOrg) {
        setError("You are not assigned as a CSPSG head.");
        setIsLoading(false);
        return;
      }
      setOrg(cspsgOrg);
      const data = await getRequirementsBySource("cspsg", cspsgOrg.id);
  ...
  async function handleTogglePublish(req: Requirement) {
    ...
    try {
      const updated = await updateRequirement(req.id, {
        is_published: !req.is_published,
      });
      setRequirements((prev) =>
        prev.map((r) =>
          r.id === req.id ? { ...r, is_published: updated.is_published } : r
        )
      );
      showToast(
        "success",
        ...
        `"${req.name}" is now ${
          updated.is_published ? "live and visible to students" : "in draft mode"
        }.`
      );
    } catch (err: unknown) {
      showToast(
  ...
  async function handleBulkPublish() {
    ...
    try {
      await Promise.all(
        drafts.map((r) => updateRequirement(r.id, { is_published: true }))
      );
      ...
      showToast(
        "error",
        "Error",
        err instanceof Error ? err.message : "Failed to bulk publish"
      );
    } finally {
    ...
    <div className="min-h-screen bg-surface-warm">
      <header className="bg-white border-b border-border-warm">
        <div className="px-4 sm:px-6 py-4 sm:py-5 flex items-center justify-between">
          <div>
            <p className="text-sm text-warm-muted">{org?.name}</p>
            <h1 className="text-2xl font-display font-bold text-cjc-navy">
              Requirements
            </h1>
          </div>
          ...
          {requirements.length === 0 ? (
            <div className="p-8 text-center">
              ...
              <p className="text-warm-muted max-w-md mx-auto mb-4">
                Add requirements all CSP students must complete for CSPSG clearance.
              </p>
              <button
                onClick={() => setIsAddModalOpen(true)}
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-gray-50 border-b border-border-warm">
                  ...
                  <th className="px-4 py-3 text-left text-xs font-semibold text-warm-muted uppercase tracking-wider w-32">
                    Applies To
                  </th>
                  <th className="px-4 py-3 text-left text-xs font-semibold text-warm-muted uppercase tracking-wider w-24">
                    Status
