...
export default function CspsgAnnouncementsPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    setError(null);
    try {
      const cspsgOrg = await getCspsgByHeadId(profile.id);
      if (!cspsgOrg) {
        setError("No CSPSG organisation found for your account.");
        setIsLoading(false);
        return;
      }
      setOrg(cspsgOrg);

      // Fetch announcements posted by this user
      const { data: myData } = await supabase
        .from("announcements")
        .select(`*, posted_by:profiles!announcements_posted_by_id_fkey(*)`)
        .eq("posted_by_id", profile.id)
        .order("created_at", { ascending: false });

      const [activeData] = await Promise.all([getActiveAnnouncements()]);

      setAnnouncements((myData as AnnouncementWithRelations[]) ?? []);

      // Received: system-wide only from others
      const received = activeData.filter(
        (a) => a.is_system_wide && a.posted_by_id !== profile.id
      );
      setReceivedAnnouncements(received);
    } catch (err) {
      console.error("Error fetching CSPSG announcements:", err);
                  ...
                  return (
                    <div
                      ...
                      className={`card p-4 cursor-pointer hover:shadow-md transition-shadow ${
                        priorityCardBorder[announcement.priority]
                      }`}
                    >
                      ...
                      <h3 className="font-semibold text-cjc-navy line-clamp-2">
                        {announcement.title}
                      </h3>
                      <p className="text-sm text-warm-muted line-clamp-2 mt-0.5">
            ...
            {!isLoading && !error && filteredAnnouncements.length === 0 && (
              <div className="card p-12 text-center">
                ...
                {announcements.length === 0 ? (
                  <>
                    <h2 className="text-lg font-semibold text-cjc-navy mb-1">
                      No Announcements Yet
                    </h2>
                    <p className="text-warm-muted mb-4">
                      Post your first announcement to all CSP students.
                    </p>
                    <button
                      onClick={handleAddAnnouncement}
                      className="btn btn-gold"
          ...
          <>
            ...
            {!isLoading && filteredReceived.length === 0 && (
              <div className="card p-12 text-center">
                ...
                <p className="text-warm-muted">
                  System-wide announcements from administrators will appear here.
                </p>
              </div>
            )}
          </>
