...
interface StudentWithClearance extends Profile {
  ...
  officeItem: ClearanceItem | null;
  clearanceStatus: ClearanceStatus;
  officeStatus: ItemStatus;
}
...
function deriveOfficeStatus(item: ClearanceItem | null): ItemStatus {
  if (!item) return null;
...
function OfficeStatusBadge({ status }: { status: ItemStatus }) {
  switch (status) {
    case "approved":
      return (
        <Badge variant="approved" size="sm">
...
export default function OfficeStudentsPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    setIsLoading(true);
    try {
      // 1. Find this user's office + fetch system settings
      const [ofc, settings] = await Promise.all([
        getOfficeByHeadId(profile.id),
        getSystemSettings(),
      ]);
      ...
      // 2. Fetch ALL students (office is system-wide)
      const studentProfiles = await getAllStudentProfiles();
      ...
      // 5. Fetch office-specific clearance items for these requests
      const requestIds = requests.map((r) => r.id);
      const officeItems = await getClearanceItemsBySourceAndRequests('office', ofc.id, requestIds);
      ...
      // 6. Build StudentWithClearance[]
      const enriched: StudentWithClearance[] = studentProfiles.map((s) => {
        const latestRequest = activeMap.get(s.id) ?? null;
        const officeItem = latestRequest ? itemByRequest.get(latestRequest.id) ?? null : null;
        return {
          ...s,
          latestRequest,
          officeItem,
          clearanceStatus: deriveOverallStatus(latestRequest),
          officeStatus: deriveOfficeStatus(officeItem),
        };
      });

      setStudents(enriched);
    } catch (err) {
      console.error("Error loading office students:", err);
  ...
  const filtered = students.filter((s) => {
    ...
    const matchesOfficeStatus = !officeStatusFilter ||
      (officeStatusFilter === "none" ? s.officeStatus === null : s.officeStatus === officeStatusFilter);
    return matchesSearch && matchesStatus && matchesOfficeStatus;
  });
            ...
            onClick={() => {
              ...
              exportToExcel(
                ...
                [
                  ...
                  { header: "Office Status", accessor: (s) => s.officeStatus ?? "—" },
                ...
                `${office?.name ?? "office"}_students_${periodLabel}`
              );
            }}
            ...
            <EmptyState
              ...
              description={
                office
                  ...
                  : "Your office could not be found."
              }
            />
          ) : filtered.length === 0 ? (
            <EmptyState
                ...
                {filtered.map((student) => {
                  ...
                  return (
                    <TableRow key={student.id}>
                      ...
                      <TableCell>
                        <OfficeStatusBadge status={student.officeStatus} />
                      </TableCell>
                      <TableCell>
                        <Button
                          variant="ghost"
