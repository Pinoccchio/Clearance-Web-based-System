...
export default function OfficeProfilePage() {
  ...
  // Fetch office on mount
  useEffect(() => {
    if (!profile?.id) return;
    setIsLoadingOrg(true);
    getOfficeByHeadId(profile.id).then((off) => {
      setOffice(off);
      setOrgForm({ description: off?.description ?? "" });
  ...
  const handleSaveOrg = async () => {
    ...
    try {
      ...
      showToast("success", "Office Updated", "Office information has been saved.");
    } catch {
      showToast("error", "Update Failed", "Could not save office info. Please try again.");
    } finally {
  ...
  const handleCancelOrg = () => {
    setOrgForm({ description: office?.description ?? "" });
    setIsEditingOrg(false);
  };
  ...
  const handleLogoUpload = async (file: File) => {
    ...
    try {
      if (office.logo_url) await deleteLogo(office.logo_url);
      const url = await uploadLogo(file, "offices", office.id);
      const updated = await updateOffice(office.id, { logo_url: url });
      setOffice(updated);
      showToast("success", "Logo Updated", "Office logo has been changed.");
    } catch {
          ...
          <div className="lg:col-span-1">
            <Card padding="lg" className="text-center">
              ...
              <div className="relative inline-block mb-4">
                {office?.logo_url ? (
                  <img
                    ...
                    onClick={() => office.logo_url && setPreviewUrl(office.logo_url)}
                  />
                ) : (
                  <div className="w-24 h-24 rounded-xl bg-cjc-navy/10 flex items-center justify-center mx-auto">
                    <Building2 className="w-12 h-12 text-cjc-navy/40" />
                  </div>
                )}
                <label
                  htmlFor="logo-upload"
              ...
              <div className="mt-6 pt-6 border-t border-gray-100 text-left">
                ...
                <Badge variant="info">Office Head</Badge>
              </div>
            </Card>
          </div>
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <Card padding="lg">
              ...
              {isLoadingOrg ? (
                ...
                <p className="text-sm text-gray-500">No office linked to your account.</p>
              )}
            </Card>
          </div>
        </div>
