...
import {
  ...
  getOfficeByHeadId,
  getProcessedClearanceItemsByOffice,
...
export default function OfficeHistoryPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      if (!off) {
        setError("Your office could not be found.");
        setItems([]);
        return;
      }
      setOffice(off);

      const data = await getProcessedClearanceItemsByOffice(off.id);
      setItems(data);
    } catch (err) {
          ...
          <Button
            ...
            onClick={() => {
              exportToExcel(
                ...
                `${office?.name ?? "office"}_clearance_history`
              );
            }}
