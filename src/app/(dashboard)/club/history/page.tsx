...
import {
  ...
  getClubByAdviserId,
  getProcessedClearanceItemsByClub,
...
function TypeBadge({ type }: { type: string }) {
  ...
  const variant = type === "graduation" ? "gold" : type === "transfer" ? "info" : "default";
...
export default function ClubHistoryPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const data = await getProcessedClearanceItemsByClub(clubData.id);
      setItems(data);
    } catch (err) {
            ...
            onClick={() => {
              exportToExcel(
                ...
                [
                  ...
                  { header: "Department", accessor: (i) => i.request?.student?.department ?? "—" },
                ...
                `${club?.name ?? "club"}_clearance_history`
              );
            }}
            ...
            <div className="bg-gray-50 rounded-xl p-4 space-y-1.5">
              ...
              {selectedItem.reviewed_by && (
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-xs text-gray-600">{club?.name ?? "Club"}</span>
                </div>
              )}
