...
export default function ClubProfilePage() {
  ...
  useEffect(() => {
    ...
    getClubByAdviserId(profile.id).then((c) => {
      setClub(c);
      setOrgForm({ description: c?.description ?? "" });
  ...
  const handleSaveOrg = async () => {
    ...
    try {
      ...
      showToast("success", "Club Updated", "Club information has been saved.");
    } catch {
      showToast("error", "Update Failed", "Could not save club info. Please try again.");
    } finally {
  ...
  const handleCancelOrg = () => {
    setOrgForm({ description: club?.description ?? "" });
    setIsEditingOrg(false);
  };
  ...
  const handleLogoUpload = async (file: File) => {
    ...
    try {
      if (club.logo_url) await deleteLogo(club.logo_url);
      const url = await uploadLogo(file, "clubs", club.id);
      const updated = await updateClub(club.id, { logo_url: url });
      setClub(updated);
      showToast("success", "Logo Updated", "Club logo has been changed.");
    } catch {
            ...
            <Card padding="lg" className="text-center">
              ...
              <div className="relative inline-block mb-4">
                {club?.logo_url ? (
                  <img
                    ...
                    onClick={() => club.logo_url && setPreviewUrl(club.logo_url)}
                  />
                ) : (
                  <div className="w-24 h-24 rounded-xl bg-cjc-navy/10 flex items-center justify-center mx-auto">
                    <Users className="w-12 h-12 text-cjc-navy/40" />
                  </div>
                )}
                <label
                  htmlFor="logo-upload"
              ...
              {isLoadingOrg ? (
                ...
                <h2 className="text-xl font-semibold text-cjc-navy">
                  {club?.name ?? "No Club Linked"}
                </h2>
              )}
              {club?.code && (
                <p className="text-gray-500 font-mono text-sm mt-1">{club.code}</p>
              )}
              ...
              <div className="flex flex-wrap gap-2 justify-center mt-3">
                ...
                {club?.type && (
                  <Badge variant={club.type === "academic" ? "info" : "warning"}>
              ...
              {club?.type === "academic" && club.department && (
                <div className="mt-4 pt-4 border-t border-gray-100 text-left">
                  ...
                  <p className="text-sm font-medium text-cjc-navy">{club.department}</p>
                </div>
              )}

              <div className="mt-4 pt-4 border-t border-gray-100 text-left">
                <p className="text-xs text-gray-500 uppercase tracking-wide mb-1">Role</p>
                <Badge variant="info">Adviser</Badge>
              </div>
            </Card>
          </div>
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <Card padding="lg">
              ...
              {isLoadingOrg ? (
                ...
                <p className="text-sm text-gray-500">No club linked to your account.</p>
              )}
            </Card>
          </div>
        </div>
