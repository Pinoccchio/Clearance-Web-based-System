...
export default function ClubRequirementsPage() {
  ...
  async function loadData(userId: string) {
    ...
    setError(null);
    try {
      const c = await getClubByAdviserId(userId);
      if (!c) {
        setError("You are not assigned as a club adviser.");
        setIsLoading(false);
        return;
      }
      setClub(c);
      const data = await getRequirementsBySource("club", c.id);
    ...
    return (
      <div className="min-h-screen bg-surface-warm">
        ...
        <div className="p-6 flex items-center justify-center">
          <div className="w-8 h-8 border-2 border-cjc-blue/30 border-t-cjc-blue rounded-full animate-spin" />
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-surface-warm">
      ...
      <header className="bg-white border-b border-border-warm">
        <div className="px-6 py-5 flex items-center justify-between">
          <div>
            <p className="text-sm text-warm-muted">{club?.name}</p>
            <h1 className="text-2xl font-display font-bold text-cjc-navy">Requirements</h1>
          </div>
          <button
            onClick={() => {
            ...
            className="flex items-center gap-2 px-4 py-2 bg-cjc-blue text-white rounded-lg hover:bg-cjc-navy transition-colors text-sm font-medium"
          >
          ...
          <div className="card p-5">
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  placeholder="e.g., Dues paid, equipment returned"
                  autoFocus
                />
                {addNameError && (
              ...
              <div>
                ...
                <textarea
                  ...
                  rows={2}
                  placeholder="Additional notes shown to members"
                />
              </div>
            ...
            <div className="flex gap-3 mt-4">
              <button
                onClick={handleAdd}
                disabled={isAdding}
                className="flex items-center gap-2 px-4 py-2 bg-cjc-blue text-white rounded-lg hover:bg-cjc-navy transition-colors text-sm font-medium disabled:opacity-50"
              >
                {isAdding ? (
          ...
          {requirements.length === 0 ? (
            <div className="p-8 text-center">
              ...
              <p className="text-warm-muted max-w-md mx-auto mb-4">
                Add requirements members must complete for club clearance.
              </p>
              <button
                onClick={() => {
                  setShowAddForm(true);
                ...
                className="flex items-center gap-2 px-4 py-2 bg-cjc-blue text-white rounded-lg hover:bg-cjc-navy transition-colors text-sm font-medium mx-auto"
              >
                <Plus className="w-4 h-4" />
                Add Requirement
              </button>
