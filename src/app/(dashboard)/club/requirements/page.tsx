...
export default function ClubRequirementsPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      setClub(clubData);
      const data = await getRequirementsBySource("club", clubData.id);
  ...
  async function handleAdd() {
    ...
    try {
      const nextOrder = requirements.length > 0
        ? Math.max(...requirements.map(r => r.order ?? 0)) + 1
        : 0;
  ...
  async function handleTogglePublish(req: Requirement) {
    ...
    try {
      ...
      showToast("success",
        ...
        `"${req.name}" is now ${updated.is_published ? "live and visible to members" : "in draft mode"}.`
      );
    } catch (err: unknown) {
          ...
          <div className="card p-5">
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  placeholder="e.g., Submit membership dues receipt"
                  autoFocus
                />
                {addNameError && (
                    ...
                    <div key={i} className="border border-gray-200 rounded-lg p-3 space-y-2 bg-gray-50">
                      <div className="flex items-center justify-between gap-2">
                        <input type="text" value={link.label}
                          ...
                          className="input-base flex-1 font-medium" placeholder="Link name (e.g. Payment Form)" />
            ...
            <div className="flex gap-3 mt-4">
              <button
                onClick={handleAdd}
                disabled={isAdding}
                className="flex items-center gap-2 px-4 py-2 bg-cjc-blue text-white rounded-lg hover:bg-cjc-blue/90 transition-colors text-sm font-medium disabled:opacity-50"
              >
                {isAdding ? (
                                ...
                                <div key={i} className="border border-gray-200 rounded-lg p-3 space-y-2 bg-gray-50">
                                  <div className="flex items-center justify-between gap-2">
                                    <input type="text" value={link.label}
                                      ...
                                      className="input-base text-sm flex-1 font-medium" placeholder="Link name (e.g. Payment Form)" />
