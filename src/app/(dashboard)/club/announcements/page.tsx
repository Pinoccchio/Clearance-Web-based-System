...
export default function ClubAnnouncementsPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    setError(null);
    try {
      const cl = await getClubByAdviserId(profile.id);
      if (!cl) {
        setError("No club found for your account.");
        setIsLoading(false);
        return;
      }
      setClub(cl);
      const [myData, activeData] = await Promise.all([
        getAnnouncementsByClub(cl.id),
        getActiveAnnouncements(),
      ]);
              ...
              <div className="card p-12 text-center">
                ...
                {announcements.length === 0 ? (
                  <>
                    ...
                    <p className="text-warm-muted mb-4">Post your first announcement to club members.</p>
                    <button onClick={handleAddAnnouncement} className="btn btn-gold">
                      <Plus className="w-4 h-4" />
                      Post Announcement
                    </button>
