...
export default function ClubClearancePage() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      if (!clubData) {
        setError("Your club could not be found.");
        setItems([]);
        return;
      }
      setClub(clubData);

      const data = await getClearanceItemsByClub(clubData.id);
      setItems(data);
    } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-surface-warm">
        ...
        <Card padding="none">
          {isLoading ? (
            ...
            <EmptyState
              ...
              description="No members have submitted clearance requests for your club yet."
            />
          ) : filtered.length === 0 ? (
            <EmptyState
                      ...
                      <TableCell>
                        <div className="flex items-center gap-3">
                          {s?.avatar_url ? (
                            <button
                              type="button"
                              onClick={() => s?.avatar_url && setLightboxUrl(s.avatar_url)}
      ...
      <Modal
        isOpen={!!selectedItem}
        onClose={() => { setSelectedItem(null); setIsConfirmDialogOpen(false); setRemarks(''); setActionType(null); }}
        className="max-w-2xl"
      >
        {selectedItem && (
          <div className="p-6 space-y-6">
            ...
            <div className="flex items-start gap-4 pr-8">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-sm text-gray-600 mt-0.5 flex items-center flex-wrap gap-2">
                  <span>{student?.department ?? "—"} · {student?.course ?? "—"} · Year {student?.year_level ?? "—"}</span>
      ...
      <ConfirmDialog
        ...
        message={
          actionType === "approved"
            ? "This will mark the member's clearance as approved. You can change this decision later if needed."
            : actionType === "rejected"
            ? "This will reject the member's clearance request. Add remarks below to explain the reason."
            : "This will put the clearance on hold. Add remarks to inform the member of next steps."
        }
        confirmText={
