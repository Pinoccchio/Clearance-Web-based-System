...
export default function DepartmentAnnouncementsPage() {
  ...
  // Load department then announcements
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      setDepartment(dept);
      const data = await getAnnouncementsByDepartment(dept.id);
  ...
  const getPostedByName = (a: AnnouncementWithRelations) => {
    ...
    return `${a.posted_by.first_name} ${a.posted_by.last_name}`;
  };
            ...
            {announcements.length === 0 ? (
              <>
                ...
                <p className="text-warm-muted mb-4">
                  Post your first announcement to students in your department.
                </p>
                <button onClick={handleAddAnnouncement} className="btn btn-gold">
                  <Plus className="w-4 h-4" />
                  Post Announcement
