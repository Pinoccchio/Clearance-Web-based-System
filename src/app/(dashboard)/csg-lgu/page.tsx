...
export default function CsgLguDashboard() {
  ...
  const loadStats = useCallback(async () => {
    ...
    try {
      ...
      // Fetch CSG LGU-specific clearance items for these requests
      ...
      console.error("Error loading CSG LGU stats:", err);
  ...
  if (error) {
    return (
      <div className="min-h-screen bg-surface-warm">
        <header className="bg-white border-b border-border-warm">
          <div className="px-6 py-5">
            <p className="text-sm text-warm-muted">{lgu?.name ?? "CSG LGU"}</p>
            <h1 className="text-2xl font-display font-bold text-cjc-navy">
              CSG LGU Dashboard
            </h1>
          </div>
    ...
    <div className="min-h-screen bg-surface-warm">
      ...
      <div className="p-6 space-y-6">
        ...
        <div className="flex flex-wrap gap-2 justify-center text-sm text-warm-muted">
          ...
          <span className="px-3 py-1 bg-white border border-gray-200 rounded-full">
            {profile?.role === "csg_lgu" ? "CSG LGU Head" : "Staff"}
          </span>
          <span className="px-3 py-1 bg-white border border-gray-200 rounded-full">
