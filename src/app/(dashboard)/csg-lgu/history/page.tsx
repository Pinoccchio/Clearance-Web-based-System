...
import {
  ...
  getCsgLguByHeadId,
  getProcessedClearanceItemsByCsgLgu,
...
export default function CsgLguHistoryPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      if (!lguData) {
        setError("Your CSG LGU could not be found.");
        setItems([]);
        return;
      }
      setLgu(lguData);

      const data = await getProcessedClearanceItemsByCsgLgu(lguData.id);
      setItems(data);
    } catch (err) {
          ...
          <Button
            ...
            onClick={() => {
              exportToExcel(
                ...
                `${lgu?.name ?? "csg_lgu"}_clearance_history`
              );
            }}
            ...
            <div className="bg-gray-50 rounded-xl p-4 space-y-1.5">
              ...
              {selectedItem.reviewed_by && (
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-xs text-gray-600">{lgu?.name ?? "CSG LGU"}</span>
                </div>
              )}
