...
export default function CsgProfilePage() {
  ...
  const loadCsg = useCallback(async () => {
    ...
    setCsg(csgData);
    setOrgForm({ description: csgData?.description ?? "" });
  ...
  useRealtimeRefresh("csg", loadCsg);
  useRealtimeRefresh("profiles", loadCsg);
  ...
  const handleSaveOrg = async () => {
    ...
    setIsSavingOrg(true);
    try {
      const updated = await updateCsg(csg.id, {
        description: orgForm.description || null,
      });
      setCsg(updated);
      setIsEditingOrg(false);
      showToast("success", "CSG Updated", "CSG information has been saved.");
    } catch {
      showToast("error", "Update Failed", "Could not save CSG info. Please try again.");
    } finally {
  ...
  const handleCancelOrg = () => {
    setOrgForm({ description: csg?.description ?? "" });
    setIsEditingOrg(false);
  };
  ...
  const handleLogoUpload = async (file: File) => {
    ...
    try {
      if (csg.logo_url) await deleteLogo(csg.logo_url);
      const url = await uploadLogo(file, "csgs", csg.id);
      const updated = await updateCsg(csg.id, { logo_url: url });
      setCsg(updated);
      showToast("success", "Logo Updated", "CSG logo has been changed.");
    } catch {
            ...
            <Card padding="lg" className="text-center">
              ...
              <div className="relative inline-block mb-4">
                {csg?.logo_url ? (
                  ...
                  <img
                    ...
                    onClick={() => csg.logo_url && setPreviewUrl(csg.logo_url)}
                  />
                ) : (
              ...
              {isLoadingOrg ? (
                ...
                <h2 className="text-xl font-semibold text-cjc-navy">
                  {csg?.name ?? "No CSG Linked"}
                </h2>
              )}
              {csg?.code && (
                <p className="text-gray-500 font-mono text-sm mt-1">{csg.code}</p>
              )}
              <Badge
                variant={csg?.status === "active" ? "success" : "neutral"}
                className="mt-3"
              >
                {csg?.status === "active" ? "Active" : "Inactive"}
              </Badge>
              ...
              <div className="mt-6 pt-6 border-t border-gray-100 text-left">
                ...
                <Badge variant="info">CSG Head</Badge>
              </div>
            </Card>
          </div>
                ...
                <div className="grid sm:grid-cols-2 gap-4">
                  ...
                  <div className="sm:col-span-2">
                    ...
                    <textarea
                      ...
                      placeholder="Enter CSG description..."
                    />
                  </div>
                </div>
              ) : (
                <p className="text-sm text-gray-500">No CSG linked to your account.</p>
              )}
            </Card>
          </div>
        </div>
