...
export default function CsgAnnouncementsPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (!csgData) {
        setError("No CSG found for your account.");
        setIsLoading(false);
        return;
      }
      setCsg(csgData);

      // Fetch announcements posted by this CSG head user
      ...
      console.error("Error fetching CSG announcements:", err);
    ...
    <div className="min-h-screen bg-surface-warm">
      <header className="bg-white border-b border-border-warm">
        <div className="px-6 py-5">
          <p className="text-sm text-warm-muted">{csg ? csg.name : "CSG"}</p>
              ...
              <div className="card p-12 text-center">
                ...
                {announcements.length === 0 ? (
                  <>
                    ...
                    <p className="text-warm-muted mb-4">Post your first announcement to all regular students.</p>
                    <button onClick={handleAddAnnouncement} className="btn btn-gold">
                      <Plus className="w-4 h-4" />
                      Post Announcement
                    </button>
