...
import { CheckSquare, Plus, Pencil, Trash2, ExternalLink, Eye, EyeOff, ScanLine } from "lucide-react";
...
export default function CsgRequirementsPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    setError(null);
    try {
      const csgData = await getCsgByHeadId(profile.id);
      if (!csgData) {
        setError("You are not assigned as a CSG head.");
        setIsLoading(false);
        return;
      }
      setCsg(csgData);
      const data = await getRequirementsBySource("csg", csgData.id);
  ...
  async function handleTogglePublish(req: Requirement) {
    ...
    try {
      ...
      setRequirements((prev) =>
        prev.map((r) => (r.id === req.id ? { ...r, is_published: updated.is_published } : r))
      );
      showToast(
        "success",
  ...
  async function handleBulkPublish() {
    ...
    try {
      ...
      showToast(
        "success",
        "All Published",
        `${drafts.length} requirement${drafts.length !== 1 ? "s" : ""} are now live.`
      );
    } catch (err: unknown) {
      ...
      <div className="min-h-screen bg-surface-warm">
        <header className="bg-white border-b border-border-warm">
          <div className="px-6 py-5">
            <p className="text-sm text-warm-muted">CSG</p>
    ...
    <div className="min-h-screen bg-surface-warm">
      <header className="bg-white border-b border-border-warm">
        <div className="px-4 sm:px-6 py-4 sm:py-5 flex items-center justify-between">
          <div>
            <p className="text-sm text-warm-muted">{csg?.name}</p>
            <h1 className="text-2xl font-display font-bold text-cjc-navy">Requirements</h1>
          </div>
          ...
          {requirements.length === 0 ? (
            <div className="p-8 text-center">
              ...
              <p className="text-warm-muted max-w-md mx-auto mb-4">
                Add requirements students must complete for CSG clearance.
              </p>
              <button
                onClick={() => setIsAddModalOpen(true)}
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-gray-50 border-b border-border-warm">
                  <th className="px-4 py-3 text-left text-xs font-semibold text-warm-muted uppercase tracking-wider w-12">#</th>
                  <th className="px-4 py-3 text-left text-xs font-semibold text-warm-muted uppercase tracking-wider">Requirement</th>
                  <th className="px-4 py-3 text-left text-xs font-semibold text-warm-muted uppercase tracking-wider w-28">Type</th>
                  <th className="px-4 py-3 text-left text-xs font-semibold text-warm-muted uppercase tracking-wider w-28">Upload</th>
                  ...
                  <th className="px-4 py-3 text-left text-xs font-semibold text-warm-muted uppercase tracking-wider w-28">Added</th>
                  <th className="px-4 py-3 text-right text-xs font-semibold text-warm-muted uppercase tracking-wider w-24">Actions</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-border-warm">
                {requirements.map((req, index) => (
                  <tr key={req.id} className="hover:bg-gray-50/50 transition-colors">
                    ...
                    <td className="px-4 py-4 text-warm-muted text-xs">{formatDate(req.created_at)}</td>
                    <td className="px-4 py-4">
