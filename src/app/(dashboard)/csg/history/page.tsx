...
export default function CsgHistoryPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      if (!csgData) {
        setError("Your CSG record could not be found.");
        setItems([]);
        return;
      }
      setCsg(csgData);
      ...
      setItems((itemsData as ClearanceItemWithDetails[]) ?? []);
    } catch (err) {
  ...
  const filtered = items.filter((item) => {
    ...
    const matchesType = typeFilter === "all" || item.request?.type === typeFilter;
              ...
              exportToExcel(
                ...
                [
                  ...
                  },
                  {
                    ...
                    accessor: (i) => {
                      const s = i.request?.student;
                      return `${s?.course ?? "—"} · Year ${s?.year_level ?? "—"}`;
                    },
                  },
                  {
                    header: "Type",
                  ...
                  },
                  {
                    header: "Period",
                    accessor: (i) =>
                      i.request ? `${i.request.academic_year} — ${i.request.semester}` : "—",
                  },
                  { header: "Status", accessor: (i) => i.status },
                  {
                    header: "Reviewed Date",
                    accessor: (i) => (i.reviewed_at ? formatDate(i.reviewed_at) : "—"),
                  },
                ],
                `${csg?.name ?? "csg"}_clearance_history`
              );
            }}
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-2 mt-2 flex-wrap">
                  ...
                  <span className="text-xs text-gray-500">
                    {request ? `${request.academic_year} — ${request.semester}` : "—"}
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className="bg-gray-50 rounded-xl p-4 space-y-1.5">
              ...
              {selectedItem.reviewed_by && (
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-xs text-gray-600">{csg?.name ?? "CSG"}</span>
                </div>
              )}
