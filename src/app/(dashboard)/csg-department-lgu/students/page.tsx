...
/**
 * Derive overall clearance status from the request (not the LGU item).
 */
...
/**
 * Derive LGU-specific item status.
 */
function deriveLguStatus(item: ClearanceItem | null): ItemStatus {
  if (!item) return null;
...
function LguStatusBadge({ status }: { status: ItemStatus }) {
  switch (status) {
    case "approved":
      return (
        <Badge variant="approved" size="sm">
...
const LGU_STATUS_OPTIONS = [
  { value: "", label: "All LGU Status" },
...
export default function CsgDepartmentLguStudentsPage() {
  ...
  const [lguStatusFilter, setLguStatusFilter] = useState("");
  ...
  const loadData = useCallback(async () => {
    ...
    setIsLoading(true);
    try {
      // 1. Find this user's LGU + fetch system settings
      const [lguData, settings] = await Promise.all([
        getCsgDepartmentLguByHeadId(profile.id),
        getSystemSettings(),
      ]);
      ...
      // 2. Fetch students belonging to this LGU's college
      // LGU.department_code matches profiles.department
      const studentProfiles = await getStudentsByDepartment(lguData.department_code);
      ...
      // 5. Fetch LGU-specific clearance items for these requests
      const requestIds = requests.map((r) => r.id);
      const lguItems = await getClearanceItemsBySourceAndRequests('csg_department_lgu', lguData.id, requestIds);

      // Map items by request_id for quick lookup
      const itemByRequest = new Map<string, ClearanceItem>();
      for (const item of lguItems) {
        itemByRequest.set(item.request_id, item);
      }

      // 6. Build StudentWithClearance[] using overall request status + LGU item status
      const enriched: StudentWithClearance[] = studentProfiles.map((s) => {
        ...
        return {
          ...
          deptStatus: deriveLguStatus(deptItem),
        };
      });

      setStudents(enriched);
    } catch (err) {
      console.error("Error loading LGU students:", err);
  ...
  const filtered = students.filter((s) => {
    ...
    const matchesLguStatus = !lguStatusFilter ||
      (lguStatusFilter === "none" ? s.deptStatus === null : s.deptStatus === lguStatusFilter);
    return matchesSearch && matchesStatus && matchesLguStatus;
  });
            ...
            onClick={() => {
              ...
              exportToExcel(
                ...
                [
                  ...
                  { header: "LGU Status", accessor: (s) => s.deptStatus ?? "—" },
                ...
                `${lgu?.name ?? "csg_department_lgu"}_students_${periodLabel}`
              );
            }}
        ...
        <Card padding="none">
          {isLoading ? (
            ...
            <EmptyState
              ...
              description={
                lgu
                  ? `No students are registered under the ${lgu.name} LGU.`
                  : "Your LGU could not be found."
              }
            />
          ) : filtered.length === 0 ? (
            <EmptyState
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead>LGU Status</TableHead>
                ...
                {filtered.map((student) => {
                  ...
                  return (
                    <TableRow key={student.id}>
                      ...
                      <TableCell>
                        <LguStatusBadge status={student.deptStatus} />
                      </TableCell>
                      <TableCell>
                        <Button
                          variant="ghost"
