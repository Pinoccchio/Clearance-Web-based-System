...
import {
  ...
  getProcessedClearanceItemsByCsgDepartmentLgu,
...
export default function CsgDepartmentLguHistoryPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const data = await getProcessedClearanceItemsByCsgDepartmentLgu(lguData.id);
      setItems(data);
    } catch (err) {
          ...
          <Button
            ...
            onClick={() => {
              exportToExcel(
                ...
                `${lgu?.name ?? "csg_department_lgu"}_clearance_history`
              );
            }}
            ...
            <div className="bg-gray-50 rounded-xl p-4 space-y-1.5">
              ...
              {selectedItem.reviewed_by && (
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-xs text-gray-600">{lgu?.name ?? "LGU"}</span>
                </div>
              )}
