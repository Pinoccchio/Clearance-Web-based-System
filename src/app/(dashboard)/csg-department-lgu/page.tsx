...
export default function CsgDepartmentLguDashboard() {
  ...
  const loadStats = useCallback(async () => {
    ...
    try {
      ...
      // Parallel fetch LGU and settings
      const [lguData, settingsData] = await Promise.all([
      ...
      // Fetch students in this LGU's college (department_code matches profiles.department)
      const students = await getStudentsByDepartment(lguData.department_code);
      ...
      // Fetch LGU-specific clearance items for these requests
      ...
      console.error("Error loading LGU stats:", err);
  ...
  if (error) {
    return (
      <div className="min-h-screen bg-surface-warm">
        <header className="bg-white border-b border-border-warm">
          <div className="px-6 py-5">
            <p className="text-sm text-warm-muted">{lgu?.name ?? "LGU"}</p>
            <h1 className="text-2xl font-display font-bold text-cjc-navy">
              LGU Dashboard
            </h1>
          </div>
      ...
      <div className="p-6 space-y-6">
        ...
        <Card padding="md">
          <CardHeader className="flex items-center justify-between">
            ...
            <Link href="/csg-department-lgu/students">
        ...
        <div className="flex flex-wrap gap-2 justify-center text-sm text-warm-muted">
          ...
          <span className="px-3 py-1 bg-white border border-gray-200 rounded-full">
            {profile?.role === "csg_department_lgu" ? "LGU Head" : "Staff"}
          </span>
          <span className="px-3 py-1 bg-white border border-gray-200 rounded-full">
