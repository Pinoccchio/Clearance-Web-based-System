...
import {
  ...
  type CsgDepartmentLgu,
} from "@/lib/supabase";
import { useToast } from "@/components/ui/Toast";
import { cn } from "@/lib/utils";

export default function CsgDepartmentLguProfilePage() {
  ...
  const loadLgu = useCallback(async () => {
    ...
    setLgu(lguData);
    setOrgForm({ description: lguData?.description ?? "" });
  ...
  useRealtimeRefresh('csg_department_lgus', loadLgu);
  useRealtimeRefresh('profiles', loadLgu);
  ...
  const handleSaveOrg = async () => {
    ...
    setIsSavingOrg(true);
    try {
      const updated = await updateCsgDepartmentLgu(lgu.id, {
        description: orgForm.description || null,
      });
      setLgu(updated);
      setIsEditingOrg(false);
      showToast("success", "LGU Updated", "LGU information has been saved.");
    } catch {
      showToast("error", "Update Failed", "Could not save LGU info. Please try again.");
    } finally {
  ...
  const handleCancelOrg = () => {
    setOrgForm({ description: lgu?.description ?? "" });
    setIsEditingOrg(false);
  };
  ...
  const handleLogoUpload = async (file: File) => {
    ...
    try {
      if (lgu.logo_url) await deleteLogo(lgu.logo_url);
      const url = await uploadLogo(file, "csg_department_lgus", lgu.id);
      const updated = await updateCsgDepartmentLgu(lgu.id, { logo_url: url });
      setLgu(updated);
      showToast("success", "Logo Updated", "LGU logo has been changed.");
    } catch {
            ...
            <Card padding="lg" className="text-center">
              ...
              <div className="relative inline-block mb-4">
                {lgu?.logo_url ? (
                  <img
                    ...
                    onClick={() => lgu.logo_url && setPreviewUrl(lgu.logo_url)}
                  />
                ) : (
              ...
              {isLoadingOrg ? (
                ...
                <h2 className="text-xl font-semibold text-cjc-navy">
                  {lgu?.name ?? "No LGU Linked"}
                </h2>
              )}
              {lgu?.code && (
                <p className="text-gray-500 font-mono text-sm mt-1">{lgu.code}</p>
              )}
              {lgu?.department_code && (
                <p className="text-gray-400 text-xs mt-0.5">Department: {lgu.department_code}</p>
              )}
              <Badge
                variant={lgu?.status === "active" ? "success" : "neutral"}
                className="mt-3"
              >
              ...
              <div className="mt-6 pt-6 border-t border-gray-100 text-left">
                ...
                <Badge variant="info">LGU Head</Badge>
              </div>
            </Card>
          </div>
            ...
            <Card padding="lg">
              ...
              {isLoadingOrg ? (
                ...
                <div className="grid sm:grid-cols-2 gap-4">
                  ...
                  <Input
                    label="Department"
                    value={lgu.department_code}
                    disabled={true}
                    helperText="Contact admin to change the department."
                  />
                  <div className="sm:col-span-2">
                    ...
                    <textarea
                      ...
                      placeholder="Enter LGU description..."
                    />
                  </div>
                </div>
              ) : (
                <p className="text-sm text-gray-500">No LGU linked to your account.</p>
              )}
            </Card>
          </div>
        </div>
