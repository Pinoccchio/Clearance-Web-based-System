...
import {
  ...
  getAnnouncementsByCsgDepartmentLgu,
...
export default function CsgDepartmentLguAnnouncementsPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (!lguData) {
        setError("No LGU found for your account.");
        setIsLoading(false);
        return;
      }
      setLgu(lguData);
      const [myData, activeData] = await Promise.all([
        getAnnouncementsByCsgDepartmentLgu(lguData.id),
        getActiveAnnouncements(),
      ]);
      ...
      console.error("Error fetching LGU announcements:", err);
  ...
  const roleLabel: Record<string, string> = {
    ...
    club: "Club", student: "Student", csg_department_lgu: "LGU",
  };
              ...
              <div className="card p-12 text-center">
                ...
                {announcements.length === 0 ? (
                  <>
                    ...
                    <p className="text-warm-muted mb-4">Post your first announcement to students in your LGU.</p>
                    <button onClick={handleAddAnnouncement} className="btn btn-gold">
                      <Plus className="w-4 h-4" />
                      Post Announcement
                    </button>
