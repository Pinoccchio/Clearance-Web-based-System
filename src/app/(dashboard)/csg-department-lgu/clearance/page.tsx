...
export default function CsgDepartmentLguClearancePage() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      if (!lguData) {
        setError("Your LGU could not be found.");
        setItems([]);
        return;
      }
      setLgu(lguData);

      const data = await getClearanceItemsByCsgDepartmentLgu(lguData.id);
      setItems(data);
    } catch (err) {
        ...
        <Card padding="none">
          {isLoading ? (
            ...
            <EmptyState
              ...
              description="No students have submitted clearance requests for your LGU yet."
            />
          ) : filtered.length === 0 ? (
            <EmptyState
