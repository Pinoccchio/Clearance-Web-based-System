...
export default function CspspDivisionSubmitPage() {
  ...
  const loadData = useCallback(async (cancelled: { value: boolean }, silent = false) => {
    ...
    try {
      ...
      if (d) {
        const reqs = await getPublishedRequirementsBySource("cspsp_division", d.id);
        if (cancelled.value || gen !== loadGenRef.current) return;
        setRequirements(reqs);

        if (active) {
          const item = await getClearanceItemForRequest(active.id, "cspsp_division", d.id);
          if (cancelled.value || gen !== loadGenRef.current) return;
  ...
  }, [profile?.id, profile?.cspsp_division]);
  ...
  if (authLoading || isLoading) {
    return (
      <div className="min-h-screen bg-gray-50">
        <Header title="Submit — CSPSP Division" subtitle="Submit your CSPSP Division clearance" />
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <Header
        title="Submit — CSPSP Division"
        subtitle={division ? `${division.name} (${division.code})` : "Submit your CSPSP Division clearance"}
      />
