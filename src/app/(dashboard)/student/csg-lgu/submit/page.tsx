...
export default function CsgLguSubmitPage() {
  ...
  const loadData = useCallback(async (cancelled: { value: boolean }, silent = false) => {
    ...
    try {
      ...
      if (d) {
        ...
        if (active) {
          const item = await getClearanceItemForRequest(active.id, "csg_lgu", d.id);
          if (cancelled.value || gen !== loadGenRef.current) return;
  ...
  if (authLoading || isLoading) {
    return (
      <div className="min-h-screen bg-gray-50">
        <Header title="Submit — CSG LGU" subtitle="Submit your CSG LGU clearance" />
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <Header
        title="Submit — CSG LGU"
        subtitle={lgu ? `${lgu.name} (${lgu.code})` : "Submit your CSG LGU clearance"}
      />
