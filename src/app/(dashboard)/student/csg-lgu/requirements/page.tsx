...
export default function CsgLguRequirementsPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const d = await getCsgLguByDepartmentCode(profile.department);
      setLgu(d);

      if (d) {
        const reqs = await getPublishedRequirementsBySource("csg_lgu", d.id);
  ...
  if (authLoading || isLoading) {
    return (
      <div className="min-h-screen bg-gray-50">
        <Header title="CSG LGU Requirements" subtitle="Requirements for your CSG LGU" />
  ...
  const requirementsBySource = lgu ? { [lgu.id]: requirements } : {};

  return (
    <div className="min-h-screen bg-gray-50">
      <Header
        title="CSG LGU Requirements"
        subtitle={lgu ? `${lgu.name} (${lgu.code})` : "Requirements for your CSG LGU"}
      />
