...
export default function CspsgSubmitPage() {
  ...
  const loadData = useCallback(async (cancelled: { value: boolean }, silent = false) => {
    ...
    try {
      const [sys, org, requests] = await Promise.all([
        getSystemSettings(),
        getActiveCspsg(),
      ...
      if (org) {
        ...
        if (active) {
          const item = await getClearanceItemForRequest(active.id, "cspsg", org.id);
          if (cancelled.value || gen !== loadGenRef.current) return;
  ...
  if (authLoading || isLoading) {
    return (
      <div className="min-h-screen bg-gray-50">
        <Header title="Submit — CSPSG" subtitle="CSP Student Government" />
