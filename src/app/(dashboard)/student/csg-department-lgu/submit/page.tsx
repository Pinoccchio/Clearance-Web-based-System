...
export default function CsgDepartmentLguSubmitPage() {
  ...
  const loadData = useCallback(async (cancelled: { value: boolean }, silent = false) => {
    ...
    try {
      const [sys, d, requests] = await Promise.all([
        getSystemSettings(),
        getCsgDepartmentLguByDepartmentCode(profile.department),
      ...
      if (d) {
        ...
        if (active) {
          const item = await getClearanceItemForRequest(active.id, "csg_department_lgu", d.id);
          if (cancelled.value || gen !== loadGenRef.current) return;
  ...
  if (authLoading || isLoading) {
    return (
      <div className="min-h-screen bg-gray-50">
        <Header title="Submit — LGU" subtitle="Submit your LGU clearance" />
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <Header
        title="Submit — LGU"
        subtitle={lgu ? `${lgu.name} (${lgu.code})` : "Submit your LGU clearance"}
      />
