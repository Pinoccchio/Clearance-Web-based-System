...
export default function CsgDepartmentLguRequirementsPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const d = await getCsgDepartmentLguByDepartmentCode(profile.department);
      setLgu(d);
  ...
  if (authLoading || isLoading) {
    return (
      <div className="min-h-screen bg-gray-50">
        <Header title="LGU Requirements" subtitle="Requirements for your LGU" />
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <Header
        title="LGU Requirements"
        subtitle={lgu ? `${lgu.name} (${lgu.code})` : "Requirements for your LGU"}
      />
