...
import { CspspDivisionFormModal } from "@/components/features/CspspDivisionFormModal";
import { ConfirmDialog } from "@/components/ui/ConfirmDialog";
import { useToast } from "@/components/ui/Toast";
import { Avatar } from "@/components/ui/Avatar";

export default function AdminCspspDivisionsPage() {
  const { showToast } = useToast();
  const [divisions, setDivisions] = useState<CspspDivisionWithHead[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [searchQuery, setSearchQuery] = useState("");

  const [isFormModalOpen, setIsFormModalOpen] = useState(false);
  const [formMode, setFormMode] = useState<"add" | "edit">("add");
  const [selectedDivision, setSelectedDivision] = useState<CspspDivisionWithHead | undefined>(undefined);

  const [isDeleteDialogOpen, setIsDeleteDialogOpen] = useState(false);
  const [divisionToDelete, setDivisionToDelete] = useState<CspspDivisionWithHead | null>(null);
  ...
  const fetchDivisions = useCallback(async () => {
    try {
      setIsLoading(true);
      const data = await getAllCspspDivisions();
      setDivisions(data);
    } catch (error) {
      console.error("Error fetching CSPSP Divisions:", error);
      showToast("error", "Error", "Failed to load CSPSP Divisions");
    } finally {
      setIsLoading(false);
    }
  }, [showToast]);
  ...
  const handleEditDivision = (div: CspspDivisionWithHead) => { setFormMode("edit"); setSelectedDivision(div); setIsFormModalOpen(true); };
  const handleDeleteClick = (div: CspspDivisionWithHead) => { setDivisionToDelete(div); setIsDeleteDialogOpen(true); };

  const handleConfirmDelete = async () => {
    if (!divisionToDelete) return;
    setIsDeleting(true);
    try {
      await deleteCspspDivision(divisionToDelete.id);
  ...
  const getHeadDisplayName = (div: CspspDivisionWithHead) => {
    ...
    <div className="min-h-screen bg-surface-warm">
      <header className="bg-white border-b border-border-warm">
        <div className="px-6 py-5">
          <p className="text-sm text-warm-muted">Manage CSPSP Divisions</p>
          <h1 className="text-2xl font-display font-bold text-cjc-navy">CSPSP Division Management</h1>
        </div>
      </header>
