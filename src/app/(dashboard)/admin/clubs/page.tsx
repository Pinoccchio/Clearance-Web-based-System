...
import { academicOrganizations, nonAcademicOrganizations } from "@/lib/mock-data";

type ClubType = "all" | "academic" | "non-academic";

export default function AdminClubsPage() {
  const [searchQuery, setSearchQuery] = useState("");
  const [typeFilter, setTypeFilter] = useState<ClubType>("all");

  // Combine both organization types with a type indicator
  const allClubs = [
    ...academicOrganizations.map((org) => ({ ...org, type: "academic" as const })),
    ...nonAcademicOrganizations.map((org) => ({ ...org, type: "non-academic" as const })),
  ];
  ...
  const academicCount = academicOrganizations.length;
  const nonAcademicCount = nonAcademicOrganizations.length;
  const totalCount = academicCount + nonAcademicCount;
    ...
    <div className="min-h-screen bg-surface-warm">
      ...
      <header className="bg-white border-b border-border-warm">
        <div className="px-6 py-5">
          ...
          <h1 className="text-2xl font-display font-bold text-cjc-navy">Clubs & Organizations</h1>
        </div>
      </header>
      ...
      <div className="p-6 space-y-6">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="card p-4 text-center">
            <p className="text-2xl font-bold text-cjc-navy">{totalCount}</p>
            <p className="text-sm text-warm-muted">Total Clubs</p>
          </div>
          <div className="card p-4 text-center">
            <p className="text-2xl font-bold text-success">{academicCount}</p>
            <p className="text-sm text-warm-muted">Academic</p>
          </div>
          <div className="card p-4 text-center">
            <p className="text-2xl font-bold text-cjc-blue">{nonAcademicCount}</p>
          ...
          <table className="w-full">
            <thead className="bg-surface-warm border-b border-border-warm">
              <tr>
                ...
                <th className="text-left py-3 px-4 text-sm font-medium text-cjc-navy">Type</th>
            ...
            <tbody className="divide-y divide-border-warm">
              {filteredClubs.map((club) => (
                <tr key={club.id} className="hover:bg-surface-warm transition-colors">
                  <td className="py-3 px-4">
                    <div className="flex items-center gap-3">
                      <div className={`w-9 h-9 rounded-lg flex items-center justify-center ${
                        club.type === "academic" ? "bg-success/10" : "bg-cjc-blue/10"
                      }`}>
                        {club.type === "academic" ? (
                      ...
                      <div>
                        ...
                        <p className="text-xs text-warm-muted">{club.description}</p>
                      </div>
                    </div>
                  </td>
                  <td className="py-3 px-4">
                    <span className={`text-xs px-2 py-1 rounded font-mono ${
                      club.type === "academic" ? "bg-success/10 text-success" : "bg-cjc-blue/10 text-cjc-blue"
                    }`}>
                      {club.code}
                    </span>
                  </td>
                  ...
                  <td className="py-3 px-4">
                    <span className="text-sm text-cjc-navy">{club.adviser || "Not assigned"}</span>
                  </td>
                  <td className="py-3 px-4">
        ...
        {filteredClubs.length === 0 && (
          <div className="card p-12 text-center">
            ...
            <p className="text-warm-muted">No clubs found</p>
          </div>
        )}
      </div>
    </div>
