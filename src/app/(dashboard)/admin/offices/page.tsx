    ...
    <div className="min-h-screen bg-surface-warm">
      ...
      <header className="bg-white border-b border-border-warm">
        <div className="px-6 py-5">
          ...
          <h1 className="text-2xl font-display font-bold text-cjc-navy">Office Management</h1>
        </div>
      </header>
      ...
      <div className="p-6 space-y-6">
        ...
        <div className="grid grid-cols-3 gap-4">
          <div className="card p-4 text-center">
            <p className="text-2xl font-bold text-cjc-navy">{offices.length}</p>
            <p className="text-sm text-warm-muted">Total Offices</p>
          </div>
          <div className="card p-4 text-center">
            <p className="text-2xl font-bold text-success">{offices.length}</p>
        ...
        <div className="card overflow-hidden">
          <table className="w-full">
            <thead className="bg-surface-warm border-b border-border-warm">
              <tr>
                <th className="text-left py-3 px-4 text-sm font-medium text-cjc-navy">Office</th>
                <th className="text-left py-3 px-4 text-sm font-medium text-cjc-navy">Code</th>
                <th className="text-left py-3 px-4 text-sm font-medium text-cjc-navy">Head</th>
            ...
            <tbody className="divide-y divide-border-warm">
              {filteredOffices.map((office) => (
                <tr key={office.id} className="hover:bg-surface-warm transition-colors">
                  <td className="py-3 px-4">
                    <div className="flex items-center gap-3">
                      <div className="w-9 h-9 rounded-lg bg-cjc-blue/10 flex items-center justify-center">
                        <Building2 className="w-4 h-4 text-cjc-blue" />
                      </div>
                      <div>
                        <p className="font-medium text-cjc-navy">{office.name}</p>
                        <p className="text-xs text-warm-muted">{office.description}</p>
                      </div>
                    </div>
                  </td>
                  <td className="py-3 px-4">
                    <span className="text-xs px-2 py-1 rounded bg-surface-warm text-cjc-navy font-mono">{office.code}</span>
                  </td>
                  <td className="py-3 px-4">
                    <span className="text-sm text-cjc-navy">{office.head || "Not assigned"}</span>
                  </td>
                  <td className="py-3 px-4">
        ...
        {filteredOffices.length === 0 && (
          <div className="card p-12 text-center">
            ...
            <p className="text-warm-muted">No offices found</p>
          </div>
        )}
      </div>
    </div>
