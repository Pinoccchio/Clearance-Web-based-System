...
import { academicOrganizations } from "@/lib/mock-data";

export default function AdminAcademicOrgsPage() {
  const [searchQuery, setSearchQuery] = useState("");

  const filteredOrgs = academicOrganizations.filter((org) =>
    ...
    <div className="min-h-screen bg-surface-warm">
      ...
      <header className="bg-white border-b border-border-warm">
        <div className="px-6 py-5">
          <p className="text-sm text-warm-muted">Manage academic organizations</p>
          <h1 className="text-2xl font-display font-bold text-cjc-navy">Academic Organizations</h1>
        </div>
      </header>
      ...
      <div className="p-6 space-y-6">
        ...
        <div className="grid grid-cols-3 gap-4">
          <div className="card p-4 text-center">
            <p className="text-2xl font-bold text-cjc-navy">{academicOrganizations.length}</p>
            <p className="text-sm text-warm-muted">Total Organizations</p>
          </div>
          <div className="card p-4 text-center">
            <p className="text-2xl font-bold text-success">{academicOrganizations.length}</p>
            ...
            <tbody className="divide-y divide-border-warm">
              {filteredOrgs.map((org) => (
                <tr key={org.id} className="hover:bg-surface-warm transition-colors">
                  ...
                  <td className="py-3 px-4">
                    <span className="text-xs px-2 py-1 rounded bg-success/10 text-success font-mono">{org.code}</span>
                  </td>
                  <td className="py-3 px-4">
                    <span className="text-xs px-2 py-1 rounded bg-cjc-navy text-white">{org.department}</span>
                  </td>
                  <td className="py-3 px-4">
        ...
        {filteredOrgs.length === 0 && (
          <div className="card p-12 text-center">
            ...
            <p className="text-warm-muted">No academic organizations found</p>
          </div>
        )}
      </div>
    </div>
