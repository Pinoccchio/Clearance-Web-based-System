...
import { CsgLguFormModal } from "@/components/features/CsgLguFormModal";
import { ConfirmDialog } from "@/components/ui/ConfirmDialog";
import { useToast } from "@/components/ui/Toast";
import { Avatar } from "@/components/ui/Avatar";

export default function AdminCsgLgusPage() {
  const { showToast } = useToast();
  const [lgus, setLgus] = useState<CsgLguWithHead[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [searchQuery, setSearchQuery] = useState("");

  const [isFormModalOpen, setIsFormModalOpen] = useState(false);
  const [formMode, setFormMode] = useState<"add" | "edit">("add");
  const [selectedLgu, setSelectedLgu] = useState<CsgLguWithHead | undefined>(undefined);

  const [isDeleteDialogOpen, setIsDeleteDialogOpen] = useState(false);
  const [lguToDelete, setLguToDelete] = useState<CsgLguWithHead | null>(null);
  ...
  const fetchLgus = useCallback(async () => {
    try {
      setIsLoading(true);
      const data = await getAllCsgLgus();
      setLgus(data);
    } catch (error) {
      console.error("Error fetching CSG LGUs:", error);
      showToast("error", "Error", "Failed to load CSG LGUs");
    } finally {
      setIsLoading(false);
    }
  }, [showToast]);
  ...
  const handleEditLgu = (lgu: CsgLguWithHead) => {
    setFormMode("edit");
    setSelectedLgu(lgu);
    setIsFormModalOpen(true);
  };

  const handleDeleteClick = (lgu: CsgLguWithHead) => {
    setLguToDelete(lgu);
  ...
  const handleConfirmDelete = async () => {
    ...
    setIsDeleting(true);
    try {
      await deleteCsgLgu(lguToDelete.id);
      showToast("success", "CSG LGU Deleted", `${lguToDelete.name} has been deleted.`);
      fetchLgus();
    } catch (error) {
      console.error("Error deleting CSG LGU:", error);
      showToast("error", "Error", "Failed to delete CSG LGU");
    } finally {
      setIsDeleting(false);
  ...
  const getHeadDisplayName = (lgu: CsgLguWithHead) => {
    ...
    <div className="min-h-screen bg-surface-warm">
      <header className="bg-white border-b border-border-warm">
        <div className="px-6 py-5">
          <p className="text-sm text-warm-muted">Manage CSG Local Government Units</p>
          <h1 className="text-2xl font-display font-bold text-cjc-navy">CSG LGU Management</h1>
        </div>
      </header>
      ...
      <div className="p-6 space-y-6">
        ...
        {isLoading && lgus.length === 0 && (
          <div className="card p-12 text-center">
            ...
            <p className="text-warm-muted">Loading CSG LGUs...</p>
          </div>
        )}
                  ...
                  <tr key={lgu.id} className="hover:bg-surface-warm transition-colors">
                    <td className="py-3 px-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="font-medium text-cjc-navy">{lgu.name}</p>
                          {lgu.description && <p className="text-xs text-warm-muted line-clamp-1">{lgu.description}</p>}
                        </div>
                      </div>
                    </td>
                    <td className="py-3 px-4">
                      <span className="text-xs px-2 py-1 rounded bg-cjc-navy text-white font-mono">{lgu.code}</span>
                    </td>
                    <td className="py-3 px-4">
        ...
        {!isLoading && filteredLgus.length === 0 && (
          <div className="card p-12 text-center">
            ...
            <p className="text-warm-muted">
              {searchQuery ? "No CSG LGUs found matching your search" : "No CSG LGUs yet. Add your first LGU to get started."}
            </p>
            {!searchQuery && (
