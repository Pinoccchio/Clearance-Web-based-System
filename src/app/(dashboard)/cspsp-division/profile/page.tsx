...
export default function CspspDivisionProfilePage() {
  ...
  const loadDivision = useCallback(async () => {
    ...
    const div = await getCspspDivisionByHeadId(profile.id);
    setDivision(div);
    setOrgForm({ description: div?.description ?? "" });
    setIsLoadingOrg(false);
  }, [profile?.id]);

  // Fetch division on mount
  useEffect(() => {
    loadDivision();
  }, [loadDivision]);

  useRealtimeRefresh('cspsp_divisions', loadDivision);
  ...
  const handleSaveOrg = async () => {
    ...
    setIsSavingOrg(true);
    try {
      const updated = await updateCspspDivision(division.id, {
        description: orgForm.description || null,
      });
      setDivision(updated);
      setIsEditingOrg(false);
      showToast("success", "Division Updated", "Division information has been saved.");
    } catch {
      showToast("error", "Update Failed", "Could not save division info. Please try again.");
    } finally {
  ...
  const handleCancelOrg = () => {
    setOrgForm({ description: division?.description ?? "" });
    setIsEditingOrg(false);
  };
  ...
  const handleLogoUpload = async (file: File) => {
    ...
    try {
      ...
      const url = await uploadLogo(file, "cspsp_divisions", division.id);
      const updated = await updateCspspDivision(division.id, { logo_url: url });
      setDivision(updated);
            ...
            <Card padding="lg" className="text-center">
              ...
              <div className="relative inline-block mb-4">
                {division?.logo_url ? (
                  <img
                    ...
                    onClick={() => division.logo_url && setPreviewUrl(division.logo_url)}
                  />
                ) : (
              ...
              {isLoadingOrg ? (
                ...
                <h2 className="text-xl font-semibold text-cjc-navy">
                  {division?.name ?? "No Division Linked"}
                </h2>
              )}
              {division?.code && (
                <p className="text-gray-500 font-mono text-sm mt-1">{division.code}</p>
              )}
              <Badge
                variant={division?.status === "active" ? "success" : "neutral"}
                className="mt-3"
              >
              ...
              <div className="mt-6 pt-6 border-t border-gray-100 text-left">
                ...
                <Badge variant="info">Division Head</Badge>
              </div>
            </Card>
          </div>
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <Card padding="lg">
              ...
              {isLoadingOrg ? (
                ...
                <p className="text-sm text-gray-500">No CSPSP division linked to your account.</p>
              )}
            </Card>
          </div>
        </div>
