...
import {
  ...
  getProcessedClearanceItemsByCspspDivision,
...
export default function CspspDivisionHistoryPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      if (!div) {
        setError("Your CSPSP division could not be found.");
        setItems([]);
        return;
      }
      setDivision(div);

      const data = await getProcessedClearanceItemsByCspspDivision(div.id);
      setItems(data);
    } catch (err) {
          ...
          <Button
            ...
            onClick={() => {
              exportToExcel(
                ...
                `${division?.name ?? "cspsp_division"}_clearance_history`
              );
            }}
            ...
            <div className="bg-gray-50 rounded-xl p-4 space-y-1.5">
              ...
              {selectedItem.reviewed_by && (
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-xs text-gray-600">{division?.name ?? "CSPSP Division"}</span>
                </div>
              )}
