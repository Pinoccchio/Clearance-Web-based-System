...
export default function CspspDivisionRequirementsPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      if (!div) {
        setError("You are not assigned as a CSPSP division head.");
        setIsLoading(false);
        return;
      }
      setDivision(div);
      const data = await getRequirementsBySource("cspsp_division", div.id);
      ...
      <div className="min-h-screen bg-surface-warm">
        <header className="bg-white border-b border-border-warm">
          <div className="px-6 py-5">
            <p className="text-sm text-warm-muted">CSPSP Division</p>
    ...
    <div className="min-h-screen bg-surface-warm">
      ...
      <header className="bg-white border-b border-border-warm">
        <div className="px-4 sm:px-6 py-4 sm:py-5 flex items-center justify-between">
          <div>
            <p className="text-sm text-warm-muted">{division?.name}</p>
            <h1 className="text-2xl font-display font-bold text-cjc-navy">Requirements</h1>
          </div>
          ...
          {requirements.length === 0 ? (
            <div className="p-8 text-center">
              ...
              <p className="text-warm-muted max-w-md mx-auto mb-4">
                Add requirements students must complete for division clearance.
              </p>
              <button
                onClick={() => setIsAddModalOpen(true)}
