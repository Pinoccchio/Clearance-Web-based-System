...
interface CsgLguFormModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSuccess: () => void;
  mode: "add" | "edit";
...
export function CsgLguFormModal({ isOpen, onClose, onSuccess, mode, lgu }: CsgLguFormModalProps) {
      ...
      const fetchData = async () => {
        try {
          const [depts, users] = await Promise.all([
            getAllDepartments(),
            mode === "edit" && lgu?.head_id ? getCsgLguRoleUsers() : getUnlinkedCsgLguUsers(),
          ]);
  ...
  const handleLogoUpload = useCallback(
    async (file: File): Promise<string> => {
      ...
      const url = await uploadLogo(file, "csg_lgus", entityId);
      return url;
    },
    [lgu?.id]
  );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (mode === "add") {
        ...
        await createCsgLgu(data);
        showToast("success", "CSG LGU Created", `${formData.name} has been added successfully.`);
      } else if (mode === "edit" && lgu) {
        if (lgu.logo_url && lgu.logo_url !== formData.logo_url) {
          await deleteLogo(lgu.logo_url);
        }
        const data: UpdateCsgLguData = {
          name: formData.name.trim(),
          code: formData.code.trim().toUpperCase(),
          department_code: formData.department_code,
        ...
        await updateCsgLgu(lgu.id, data);
        showToast("success", "CSG LGU Updated", `${formData.name} has been updated successfully.`);
      }
      onSuccess();
      onClose();
    } catch (error: unknown) {
    ...
    <Modal isOpen={isOpen} onClose={onClose} className="max-w-2xl">
      <div className="p-6">
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            <h2 className="text-lg font-semibold text-cjc-navy">
              {mode === "add" ? "Add New CSG LGU" : "Edit CSG LGU"}
            </h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <Select label="Linked Account (Head)" name="head_id" value={formData.head_id} onChange={handleChange} options={headOptions} disabled={loadingHeads} helperText={loadingHeads ? "Loading available accounts..." : "Select a user with 'CSG LGU' role to manage this LGU"} />
          ...
          <div className="flex gap-3 pt-4">
            ...
            <Button type="submit" variant="gold" className="flex-1" isLoading={isLoading}>
              {mode === "add" ? "Create CSG LGU" : "Save Changes"}
            </Button>
          </div>
        </form>
      </div>
