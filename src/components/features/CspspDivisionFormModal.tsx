...
import {
  ...
  createCspspDivision,
  updateCspspDivision,
  CreateCspspDivisionData,
  UpdateCspspDivisionData,
  getUnlinkedCspspDivisionUsers,
  getCspspDivisionRoleUsers,
  getAllDepartments,
  Department,
} from "@/lib/supabase";
...
interface CspspDivisionFormModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSuccess: () => void;
  mode: "add" | "edit";
  division?: CspspDivisionWithHead;
}
...
export function CspspDivisionFormModal({ isOpen, onClose, onSuccess, mode, division }: CspspDivisionFormModalProps) {
      ...
      const fetchData = async () => {
        try {
          const [depts, users] = await Promise.all([
            getAllDepartments(),
            mode === "edit" && division?.head_id ? getCspspDivisionRoleUsers() : getUnlinkedCspspDivisionUsers(),
          ]);
  ...
  const handleLogoUpload = useCallback(async (file: File): Promise<string> => {
    ...
    return await uploadLogo(file, "cspsp_divisions", entityId);
  }, [division?.id]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (mode === "add") {
        const data: CreateCspspDivisionData = {
        ...
        await createCspspDivision(data);
        ...
        const data: UpdateCspspDivisionData = {
          name: formData.name.trim(), code: formData.code.trim().toUpperCase(),
          department_code: formData.department_code,
          description: formData.description.trim() || null,
          head_id: formData.head_id || null, logo_url: formData.logo_url || null, status: formData.status,
        };
        await updateCspspDivision(division.id, data);
      ...
      <div className="p-4 sm:p-6">
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500">{mode === "add" ? "Create a new CSPSP Division" : "Update division information"}</p>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <Select label="Linked Account (Head)" name="head_id" value={formData.head_id} onChange={handleChange} options={headOptions} disabled={loadingHeads} helperText={loadingHeads ? "Loading available accounts..." : "Select a user with 'CSPSP Division' role to manage this division"} />
